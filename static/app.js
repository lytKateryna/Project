...
async function searchByKeyword() {
    ...
    try {
        const response = await fetch(
            `/films/search/keyword?query=${encodeURIComponent(keyword)}`
        );
...
function searchByYearRange() {
    ...
    if (from > to) {
        document.getElementById("results").innerHTML = "<p>From year cannot be greater than to year</p>";
        return;
    }
    ...
    fetch(`/films/search/year_range?year_from=${encodeURIComponent(from)}&year_to=${encodeURIComponent(to)}`)
        .then(res => {
            if (!res.ok) throw new Error('Server error: ' + res.status);
            return res.json();
        })
