...
def get_films_by_year_range(year_from: int, year_to: int, category_id: int = None, limit: int = 10, offset: int = 0) -> list[dict]:
    if category_id is not None:
        sql = """
            SELECT f.film_id, f.title, f.release_year, f.length, f.rating, '/static/no-poster.png' AS poster_url
            FROM film AS f
            JOIN film_category AS fc ON fc.film_id = f.film_id
